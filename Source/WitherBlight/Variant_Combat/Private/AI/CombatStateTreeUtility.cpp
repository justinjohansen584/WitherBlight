...
#include "AI/CombatStateTreeUtility.h"
...
EStateTreeRunStatus FStateTreeComboAttackTask::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	...
	if (Transition.ChangeType == EStateTreeStateChangeType::Changed)
	{
		...
		InstanceData.Character->DoAILightAttack();
	}
...
EStateTreeRunStatus FStateTreeGetPlayerInfoTask::Tick(FStateTreeExecutionContext& Context, const float DeltaTime) const
{
	...
	IAbilitySystemInterface* ASCCharacter = Cast<IAbilitySystemInterface>(InstanceData.Character);
	if (ASCCharacter)
	{
		const UGSAttributeSetBase* BaseSet = Cast<UGSAttributeSetBase>(ASCCharacter->GetAbilitySystemComponent()->GetAttributeSet(UGSAttributeSetBase::StaticClass()));
		if (BaseSet)
		{
			InstanceData.CurrentHP = BaseSet->GetHealth();
		}
	}
	return EStateTreeRunStatus::Running;
}
