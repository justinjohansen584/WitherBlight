...
void ACombatEnemy::DoAILightAttack()
{
	...
	TargetLightCount = FMath::RandRange(1, LightSectionNames.Num() - 1);
...
void ACombatEnemy::BlockGathered(int32 newTotal)
{
	//Right now this does nothing
}

FCharacterStats ACombatEnemy::GetCharacterStats()
{
...
void ACombatEnemy::CheckLight()
{
	// increase the Light counter
	++CurrentLightAttack;

	// do we still have attacks to play in this string?
	if (CurrentLightAttack < TargetLightCount)
	{
		// jump to the next attack section
		if (UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance())
		{
			AnimInstance->Montage_JumpToSection(LightSectionNames[CurrentLightAttack], LightAttackMontage);
		}
	}
}
...
void ACombatEnemy::ApplyDamage(float Damage, AActor* DamageCauser, const FVector& DamageLocation, const FVector& DamageImpulse)
{
	...
	if (ActualDamage > 0.0f)
	{
		...
		if (UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance())
		{
			AnimInstance->Montage_Stop(0.1f, LightAttackMontage);
...
void ACombatEnemy::HandleDamageTaken(float DamageAmount, const FGameplayTagContainer& SourceTags)
{
...
bool ACombatEnemy::IsAlive() const
{
